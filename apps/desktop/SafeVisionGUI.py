...
class SafeVisionGUI(QMainWindow):
    ...
    def create_basic_options_tab(self):
        ...
        detector_group.setFixedHeight(255)
        detector_layout = QVBoxLayout(detector_group)

        saved_detectors = {
            item.strip().lower()
            for item in str(self.settings.get("detectors", "nude,age,gender")).replace("+", ",").split(",")
            if item.strip()
        }
        detector_row = QHBoxLayout()
        detector_row.addWidget(QLabel("Content:"))
        self.detectors_combo = QComboBox()
        self.detectors_combo.addItems(["nude", "objects", "both"])
        if "nude" in saved_detectors and "objects" in saved_detectors:
            self.detectors_combo.setCurrentText("both")
        elif "objects" in saved_detectors and "nude" not in saved_detectors:
            self.detectors_combo.setCurrentText("objects")
        else:
            self.detectors_combo.setCurrentText("nude")
        ...
        self.age_detection_toggle = ModernToggle("Detect estimated underage people")
        self.age_detection_toggle.setChecked("age" in saved_detectors or not saved_detectors)
        detector_layout.addWidget(self.age_detection_toggle)

        self.gender_detection_toggle = ModernToggle("Classify model-reported gender")
        self.gender_detection_toggle.setChecked("gender" in saved_detectors or not saved_detectors)
        detector_layout.addWidget(self.gender_detection_toggle)

        age_row = QHBoxLayout()
        age_row.addWidget(QLabel("Underage below:"))
        self.underage_age_spin = QDoubleSpinBox()
        self.underage_age_spin.setRange(1.0, 100.0)
        self.underage_age_spin.setDecimals(1)
        self.underage_age_spin.setValue(float(self.settings.get("underage_age", 18.0)))
        self.underage_age_spin.setSuffix(" estimated years")
        age_row.addWidget(self.underage_age_spin)
        age_row.addStretch()
        detector_layout.addLayout(age_row)

        review_row = QHBoxLayout()
        review_row.addWidget(QLabel("Review margin:"))
        self.age_review_margin_spin = QDoubleSpinBox()
        self.age_review_margin_spin.setRange(0.0, 25.0)
        self.age_review_margin_spin.setDecimals(1)
        self.age_review_margin_spin.setValue(float(self.settings.get("age_review_margin", 3.0)))
        self.age_review_margin_spin.setSuffix(" years")
        review_row.addWidget(self.age_review_margin_spin)
        review_row.addStretch()
        detector_layout.addLayout(review_row)

        layout.addWidget(detector_group)

        protection_group = QGroupBox("Child Protection Policy")
        protection_group.setFixedHeight(220)
        protection_layout = QVBoxLayout(protection_group)

        self.block_compound_toggle = ModernToggle("Block qualified NSFW + estimated underage")
        self.block_compound_toggle.setChecked(self.settings.get("block_if_nsfw_and_child", True))
        protection_layout.addWidget(self.block_compound_toggle)

        self.block_any_child_toggle = ModernToggle("Block any estimated underage face")
        self.block_any_child_toggle.setChecked(self.settings.get("block_if_child", False))
        protection_layout.addWidget(self.block_any_child_toggle)

        self.block_age_review_toggle = ModernToggle("Block age estimates in the review margin")
        self.block_age_review_toggle.setChecked(self.settings.get("block_on_age_review", False))
        protection_layout.addWidget(self.block_age_review_toggle)

        risk_row = QHBoxLayout()
        risk_row.addWidget(QLabel("Compound NSFW risk:"))
        self.child_nsfw_risk_combo = QComboBox()
        self.child_nsfw_risk_combo.addItems(["LOW", "MODERATE", "HIGH", "CRITICAL"])
        self.child_nsfw_risk_combo.setCurrentText(str(self.settings.get("child_nsfw_min_risk", "HIGH")).upper())
        risk_row.addWidget(self.child_nsfw_risk_combo)
        risk_row.addStretch()
        protection_layout.addLayout(risk_row)

        confidence_row = QHBoxLayout()
        confidence_row.addWidget(QLabel("Minimum confidence:"))
        self.child_nsfw_confidence_spin = QDoubleSpinBox()
        self.child_nsfw_confidence_spin.setRange(0.0, 1.0)
        self.child_nsfw_confidence_spin.setSingleStep(0.05)
        self.child_nsfw_confidence_spin.setDecimals(2)
        self.child_nsfw_confidence_spin.setValue(float(self.settings.get("child_nsfw_min_confidence", 0.5)))
        confidence_row.addWidget(self.child_nsfw_confidence_spin)
        confidence_row.addStretch()
        protection_layout.addLayout(confidence_row)
        ...
        privacy_group = QGroupBox("Output Privacy and Full Cover")
        privacy_group.setFixedHeight(340)
        privacy_layout = QVBoxLayout(privacy_group)

        self.final_boxes_toggle = ModernToggle("Draw detection boxes on the final output")
        self.final_boxes_toggle.setChecked(self.settings.get("boxes_on_final", False))
        privacy_layout.addWidget(self.final_boxes_toggle)
        self.boxes_radio.toggled.connect(
            lambda checked: self.final_boxes_toggle.setChecked(True) if checked else None
        )

        self.save_boxes_copy_toggle = ModernToggle("Save unredacted reviewer boxes copy")
        self.save_boxes_copy_toggle.setChecked(self.settings.get("save_boxes_copy", False))
        privacy_layout.addWidget(self.save_boxes_copy_toggle)

        self.save_blur_copy_toggle = ModernToggle("Save separate clean regional-censor image")
        self.save_blur_copy_toggle.setChecked(self.settings.get("save_blur_copy", True))
        privacy_layout.addWidget(self.save_blur_copy_toggle)

        self.force_full_cover_toggle = ModernToggle("Force a whole-image/video cover")
        self.force_full_cover_toggle.setChecked(self.settings.get("force_full_cover", False))
        privacy_layout.addWidget(self.force_full_cover_toggle)

        cover_mode_row = QHBoxLayout()
        cover_mode_row.addWidget(QLabel("Cover mode:"))
        self.full_cover_mode_combo = QComboBox()
        self.full_cover_mode_combo.addItems(["blur", "gray", "black", "color"])
        self.full_cover_mode_combo.setCurrentText(self.settings.get("full_cover_mode", "blur"))
        cover_mode_row.addWidget(self.full_cover_mode_combo)
        cover_mode_row.addWidget(QLabel("Color:"))
        saved_cover_color = self.settings.get("full_cover_color", [96, 96, 96])
        if isinstance(saved_cover_color, str):
            try:
                saved_cover_color = [int(part.strip()) for part in saved_cover_color.split(",")]
            except ValueError:
                saved_cover_color = [96, 96, 96]
        if not isinstance(saved_cover_color, (list, tuple)) or len(saved_cover_color) != 3:
            saved_cover_color = [96, 96, 96]
        self.full_cover_color_button = ColorPickerButton(list(saved_cover_color))
        cover_mode_row.addWidget(self.full_cover_color_button)
        cover_mode_row.addStretch()
        privacy_layout.addLayout(cover_mode_row)

        self.full_cover_text_toggle = ModernToggle("Show the policy reason at the center")
        self.full_cover_text_toggle.setChecked(self.settings.get("full_cover_text", True))
        privacy_layout.addWidget(self.full_cover_text_toggle)

        message_row = QHBoxLayout()
        message_row.addWidget(QLabel("Optional message:"))
        self.full_cover_message_edit = QLineEdit()
        self.full_cover_message_edit.setPlaceholderText("Leave empty for the automatic reason-specific text")
        self.full_cover_message_edit.setText(self.settings.get("full_cover_message", ""))
        message_row.addWidget(self.full_cover_message_edit)
        privacy_layout.addLayout(message_row)
    ...
    def create_advanced_options_tab(self):
        ...
        self.rule_file_edit.setText(self.settings.get("rule_file", "BlurException.rule"))
    ...
    def build_command(self):
        ...
        age_gender_model_path = self.settings.get("age_gender_model", "Models/onnx-communityage-gender-prediction-ONNX.onnx")
        ...
        if age_gender_model_path and not os.path.isabs(age_gender_model_path):
            age_gender_model_path = str(base_dir / age_gender_model_path)

        content_selection = self.detectors_combo.currentText()
        selected_detectors = ["nude", "objects"] if content_selection == "both" else [content_selection]
        if self.age_detection_toggle.isChecked():
            selected_detectors.append("age")
        if self.gender_detection_toggle.isChecked():
            selected_detectors.append("gender")
        detector_value = ",".join(dict.fromkeys(selected_detectors))

        def append_rule_policy_and_cover_options(command):
            rule_file = self.rule_file_edit.text().strip()
            if rule_file:
                command.extend(["-e", rule_file])

            command.append(
                "--block-if-nsfw-and-child"
                if self.block_compound_toggle.isChecked()
                else "--no-block-if-nsfw-and-child"
            )
            command.append(
                "--block-if-child"
                if self.block_any_child_toggle.isChecked()
                else "--no-block-if-child"
            )
            command.append(
                "--block-on-age-review"
                if self.block_age_review_toggle.isChecked()
                else "--no-block-on-age-review"
            )
            command.extend(["--child-nsfw-min-risk", self.child_nsfw_risk_combo.currentText()])
            command.extend(
                ["--child-nsfw-min-confidence", str(self.child_nsfw_confidence_spin.value())]
            )
            command.extend(["--full-cover-mode", self.full_cover_mode_combo.currentText()])
            cover_color = self.full_cover_color_button.get_color()
            command.extend(
                ["--full-cover-color", f"{cover_color[0]},{cover_color[1]},{cover_color[2]}"]
            )
            command.append(
                "--full-cover-text"
                if self.full_cover_text_toggle.isChecked()
                else "--no-full-cover-text"
            )
            cover_message = self.full_cover_message_edit.text().strip()
            if cover_message:
                command.extend(["--full-cover-message", cover_message])
            if self.force_full_cover_toggle.isChecked():
        ...
        if file_ext in ['.jpg', '.jpeg', '.png', '.bmp']:
            ...
            command.extend(["--detectors", detector_value])
            ...
            command.extend(["--age-gender-model", age_gender_model_path])
            command.extend(["--underage-age", str(self.underage_age_spin.value())])
            command.extend(["--age-review-margin", str(self.age_review_margin_spin.value())])
            append_rule_policy_and_cover_options(command)
            ...
            command.append("--boxes" if self.final_boxes_toggle.isChecked() else "--no-boxes")
            command.append(
                "--save-boxes-copy"
                if self.save_boxes_copy_toggle.isChecked()
                else "--no-save-boxes-copy"
            )
            command.append(
                "--save-blur-copy"
                if self.save_blur_copy_toggle.isChecked()
                else "--no-save-blur-copy"
            )
            ...
            elif self.boxes_radio.isChecked() or self.final_boxes_toggle.isChecked():
    ...
    def _load_settings(self):
        ...
        settings_file = PROJECT_ROOT / "safevision_settings.json"
        default_settings = {
            "output_dir": "",
            "ffmpeg_path": "",
            "codec": "mp4v",
            ...
            "child_nsfw_min_risk": "HIGH",
            "child_nsfw_min_confidence": 0.5,
            "rule_file": "BlurException.rule",
            "mask_shape": "rectangle",
            "mask_color": [0, 0, 0],
            "blur_strength": 23,
            "boxes_on_final": False,
    ...
    def _save_settings(self):
        ...
        settings = {
            ...
            "detectors": ",".join(
                dict.fromkeys(
                    (["nude", "objects"] if self.detectors_combo.currentText() == "both" else [self.detectors_combo.currentText()])
                    + (["age"] if self.age_detection_toggle.isChecked() else [])
                    + (["gender"] if self.gender_detection_toggle.isChecked() else [])
                )
            ),
            ...
            "age_gender_model": self.settings.get("age_gender_model", "Models/onnx-communityage-gender-prediction-ONNX.onnx"),
            "underage_age": self.underage_age_spin.value(),
            "age_review_margin": self.age_review_margin_spin.value(),
            "block_if_nsfw_and_child": self.block_compound_toggle.isChecked(),
            "block_if_child": self.block_any_child_toggle.isChecked(),
            "block_on_age_review": self.block_age_review_toggle.isChecked(),
            "child_nsfw_min_risk": self.child_nsfw_risk_combo.currentText(),
            "child_nsfw_min_confidence": self.child_nsfw_confidence_spin.value(),
            "rule_file": self.rule_file_edit.text().strip() or "BlurException.rule",
            ...
            "boxes_on_final": self.final_boxes_toggle.isChecked(),
            "save_boxes_copy": self.save_boxes_copy_toggle.isChecked(),
            "save_blur_copy": self.save_blur_copy_toggle.isChecked(),
            "force_full_cover": self.force_full_cover_toggle.isChecked(),
            "full_cover_mode": self.full_cover_mode_combo.currentText(),
            "full_cover_color": self.full_cover_color_button.get_color(),
            "full_cover_text": self.full_cover_text_toggle.isChecked(),
            "full_cover_message": self.full_cover_message_edit.text().strip(),
        ...
        try:
            with open(PROJECT_ROOT / "safevision_settings.json", 'w') as f:
    ...
    def auto_detect_rule_files(self):
        ...
        current_dir = str(PROJECT_ROOT)
        preferred_rule = self.rule_file_edit.text().strip()
        if preferred_rule and not os.path.isabs(preferred_rule):
            preferred_rule = os.path.join(current_dir, preferred_rule)
        ...
        # Update UI
        if rule_candidates:
            default_rule = os.path.join(current_dir, "BlurException.rule")
            selected_rule = preferred_rule if preferred_rule and os.path.isfile(preferred_rule) else default_rule
            if not os.path.isfile(selected_rule):
                selected_rule = sorted(rule_candidates)[0]
            self.rule_file_edit.setText(selected_rule)
    ...
    def get_video_script_path(self):
        ...
        if custom_path and os.path.exists(custom_path):
            return custom_path
        # Fallback to current logic
        return str(PROJECT_ROOT / "video.py")
