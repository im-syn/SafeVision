...
STREAMER_CONFIG = {
    ...
    'ENABLED_DETECTORS': 'nude,age,gender', # NSFW plus child/gender checks by default
    'AGE_GENDER_MODEL': str(default_age_gender_model_path()),
    'UNDERAGE_AGE': None,                    # Defaults to BlurException.rule
    'AGE_REVIEW_MARGIN': None,               # Defaults to BlurException.rule
    'MIN_FACE_SIZE': 32,
    'FACE_PADDING': 0.18,
    'SHOW_DEMOGRAPHICS': False,
    'PROTECTION_RULES': str(PROJECT_ROOT / 'BlurException.rule'),
...
class StreamProcessor:
    ...
    def __init__(self, config=None):
        ...
        self.enabled_detectors = [
            name
            for name in parse_detector_selection(self.config.get('ENABLED_DETECTORS', 'nude,age,gender'))
            if name in {'nude', 'age', 'gender'}
        ]
        if not self.enabled_detectors:
            raise ValueError("No streamer checks enabled. Use --detectors nsfw,age,gender, demographics, or all.")
        
        # Initialize components
        self.detector = StreamerDetector() if 'nude' in self.enabled_detectors else None
        self.protection_rules = load_protection_rules(
            self.config.get('PROTECTION_RULES', str(PROJECT_ROOT / 'BlurException.rule'))
        )
        if self.config.get('UNDERAGE_AGE') is None:
            self.config['UNDERAGE_AGE'] = float(self.protection_rules['UNDERAGE_AGE'])
        if self.config.get('AGE_REVIEW_MARGIN') is None:
            self.config['AGE_REVIEW_MARGIN'] = float(self.protection_rules['AGE_REVIEW_MARGIN'])
        self.demographic_detector = None
        if 'age' in self.enabled_detectors or 'gender' in self.enabled_detectors:
            providers = None if self.config.get('GPU_ACCELERATION', True) else ['CPUExecutionProvider']
            self.demographic_detector = AgeGenderDetector(
                model_path=self.config.get('AGE_GENDER_MODEL') or default_age_gender_model_path(),
                providers=providers,
                min_face_size=self.config.get('MIN_FACE_SIZE', 32),
                face_padding=self.config.get('FACE_PADDING', 0.18),
            )
        ...
        self.child_protection_until = 0.0
    ...
    def process_frame(self, frame):
        ...
        if should_process_ai:
            if self.detector is not None and self.detector.onnx_session is not None:
                raw_detections = self.detector.detect_frame(frame)
            self.latest_demographics = self.analyze_demographics(frame, raw_detections)
            nsfw_gate = protection_nsfw_summary(raw_detections, self.protection_rules)
            self.latest_protection_policy = evaluate_protection_policy(
                nsfw_gate['detected'],
                self.latest_demographics,
            ...
            self.latest_protection_policy['nsfw_gate'] = nsfw_gate
            if self.latest_protection_policy['blocked']:
                self.child_protection_until = time.time() + float(self.config['SAFE_MODE_TIMEOUT'])
                self.enter_safe_mode('CHILD_PROTECTION_POLICY')
        ...
        if should_process_ai and self.config['LOG_DETECTIONS'] and (
            raw_detections or self.latest_demographics.get('faces')
        ):
        ...
        if self.config.get('SHOW_DEMOGRAPHICS') and not self.latest_protection_policy.get('blocked'):
            processed_frame = self.apply_demographic_overlays(processed_frame)

        if time.time() < self.child_protection_until:
            processed_frame = self.apply_child_protection_block(processed_frame)
    ...
    def analyze_demographics(self, frame, raw_nsfw_detections):
        ...
        return self.demographic_detector.analyze_frame(
            ...
            age_threshold=self.config.get('UNDERAGE_AGE', 18.0),
            review_margin=self.config.get('AGE_REVIEW_MARGIN', 3.0),
            face_source='safevision_nsfw_faces' if face_boxes else None,
        )

    def apply_demographic_overlays(self, frame):
        output = frame.copy()
        for face in self.latest_demographics.get('faces', []):
            box = face.get('bounding_box') or {}
            x = int(box.get('x', 0))
            y = int(box.get('y', 0))
            w = int(box.get('width', 0))
            h = int(box.get('height', 0))
            color = (0, 128, 255) if face.get('is_underage') else (255, 180, 0)
            cv2.rectangle(output, (x, y), (x + w, y + h), color, 2)
            parts = []
            if face.get('age_estimate') is not None:
                prefix = 'CHILD' if face.get('is_underage') else ('REVIEW' if face.get('review_required') else 'age')
                parts.append(f"{prefix}~{face['age_estimate']:.1f}")
            if face.get('gender'):
                parts.append(f"{face['gender']} {float(face.get('gender_confidence') or 0):.0%}")
            if parts:
                cv2.putText(output, ' | '.join(parts), (x, max(18, y - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2)
        return output

    def apply_child_protection_block(self, frame):
        output = np.zeros_like(frame)
        message = 'SAFEVISION CHILD-PROTECTION BLOCK'
        size = cv2.getTextSize(message, cv2.FONT_HERSHEY_SIMPLEX, 1.0, 2)[0]
        x = max(10, (output.shape[1] - size[0]) // 2)
        y = max(40, output.shape[0] // 2)
        cv2.putText(output, message, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2, cv2.LINE_AA)
        return output
    ...
    def apply_overlays(self, frame, tracks):
        ...
        if self.config['STREAM_SAFE_OVERLAY']:
            ...
            if self.latest_demographics.get('enabled'):
                underage = 'YES' if self.latest_demographics.get('underage_detected') else 'NO'
                policy = self.latest_protection_policy.get('verdict', 'ALLOW')
                cv2.putText(
                    frame,
                    f"Estimated underage: {underage} | policy: {policy}",
                    (10, 58),
                    font,
                    0.6,
                    (0, 180, 255) if underage == 'YES' else (180, 255, 180),
                    2,
                )
    ...
    def log_detections(self, detections):
        ...
        log_entry = {
            ...
            'checks': list(self.enabled_detectors),
            'demographics': self.latest_demographics,
            'protection_policy': self.latest_protection_policy,
        }
...
def parse_streamer_args():
    ...
    parser.add_argument("--detectors", default="nude,age,gender",
                       help="Checks to run: nsfw/nude, age, gender, demographics, or all")
    parser.add_argument("--age-gender-model", default=str(default_age_gender_model_path()),
                       help="Path to the estimated-age/gender ONNX model")
    parser.add_argument("--underage-age", type=float, default=None,
                       help="Estimated underage threshold; defaults to BlurException.rule")
    parser.add_argument("--age-review-margin", type=float, default=None,
                       help="Review band above the threshold; defaults to BlurException.rule")
    ...
    parser.add_argument("--show-demographics", action="store_true",
                       help="Draw non-censoring estimated-age/gender face boxes")
    parser.add_argument("--protection-rules", default=str(PROJECT_ROOT / "BlurException.rule"),
                       help="Child-protection rule file")
...
def main():
    ...
    config['ENABLED_DETECTORS'] = args.detectors
    config['AGE_GENDER_MODEL'] = args.age_gender_model
    config['UNDERAGE_AGE'] = None if args.underage_age is None else max(1.0, min(100.0, args.underage_age))
    config['AGE_REVIEW_MARGIN'] = None if args.age_review_margin is None else max(0.0, min(25.0, args.age_review_margin))
    config['MIN_FACE_SIZE'] = max(16, args.min_face_size)
    config['FACE_PADDING'] = max(0.0, min(1.0, args.face_padding))
    config['SHOW_DEMOGRAPHICS'] = args.show_demographics
    config['PROTECTION_RULES'] = args.protection_rules
