...
DEFAULT_RULE_FILE = str(PROJECT_ROOT / "BlurException.rule")
...
from age_gender_detector import (
    AgeGenderDetector as SharedAgeGenderDetector,
...
class LiveNudeDetector:
    ...
    def auto_load_blur_rules(self):
        ...
        rule_file_path = DEFAULT_RULE_FILE
...
class GenderAgeDetector:
    def __init__(self, providers=None, model_path=None, underage_age=18.0, review_margin=3.0):
        """Wrap the shared estimated-age/gender model for live face crops."""
        print("Initializing Gender/Age Detector...")

        self.underage_age = float(underage_age)
        self.review_margin = float(review_margin)
        self.detector = SharedAgeGenderDetector(
            model_path=model_path or default_age_gender_model_path(),
            providers=providers,
        )
        self.detector.load()
        self.session = self.detector.session
        self.input_name = self.detector.input_name
        self.input_shape = [None, 3, 224, 224]
        print(f"Gender/Age Detector initialized with {self.detector.model_path}")

    def predict_gender_age(self, face_region):
        """Return model gender, estimated age, gender confidence, and no age confidence."""
        ...
        try:
            result = self.detector.analyze_frame(
                face_region,
                face_boxes=[[0, 0, face_region.shape[1], face_region.shape[0]]],
                age_enabled=True,
                gender_enabled=True,
                age_threshold=self.underage_age,
                review_margin=self.review_margin,
                face_source="safevision_live_face",
            )
            if result.get("faces"):
                face = result["faces"][0]
                return face.get("gender", "").title(), face.get("age_estimate"), face.get("gender_confidence", 0.0), None
            return None, None, 0.0, 0.0
        except Exception as e:
...
class LiveProcessor:
    def __init__(
        ...
        self.protection_rules = load_protection_rules(rules_file or DEFAULT_RULE_FILE)
        self.underage_age = float(
            underage_age if underage_age is not None else self.protection_rules["UNDERAGE_AGE"]
        )
        ...
        if enable_gender_detection:
            self.gender_detector = GenderAgeDetector(
                ...
                underage_age=self.underage_age,
        ...
        while self.running:
            try:
                ...
                if self.enable_gender_detection:
                    face_results = self.analyze_faces_for_gender_age(frame, detections)
                else:
                    face_results = {}
                    self.latest_demographics = {
                        "enabled": False,
                ...
                demographics = dict(self.latest_demographics)
                
                # Put results back (non-blocking to avoid hanging)
                try:
                    self.result_queue.put((frame, detections, face_results, demographics), timeout=0.1)
                except queue.Full:
    ...
    def analyze_faces_for_gender_age(self, frame, detections):
        """Batch all face crops through the shared demographic model."""
        ...
        face_boxes = face_boxes_from_detections(detections, width=frame.shape[1], height=frame.shape[0])
        self.latest_demographics = self.gender_detector.detector.analyze_frame(
            frame,
            ...
            age_threshold=self.underage_age,
            review_margin=self.age_review_margin,
            face_source="safevision_live_faces" if face_boxes else None,
        )
        face_results = {}
        for index, face in enumerate(self.latest_demographics.get("faces", []), start=1):
            box = face.get("bounding_box") or {}
            face_results[f"face_{index}"] = {
                "box": [box.get("x", 0), box.get("y", 0), box.get("width", 0), box.get("height", 0)],
                "gender": str(face.get("gender") or "").title(),
                "age": face.get("age_estimate"),
                ...
                "age_confidence": None,
                "detected_label": face.get("face_source"),
            }
        return face_results
    ...
    def check_alert_conditions(self, detections):
        ...
        if self.latest_protection_policy["blocked"]:
            should_alert = True
            alert_msg = "CHILD PROTECTION: " + ", ".join(self.latest_protection_policy["reasons"])
    ...
    def draw_status_overlay(self, frame):
        ...
        if self.enable_gender_detection:
            status_lines.append(f"Age/Gender Checks: {'ON' if self.gender_detector and self.gender_detector.session else 'FAILED'}")
        ...
        if self.enable_gender_detection and hasattr(self, 'face_analysis_results'):
            ...
            for face_id, result in self.face_analysis_results.items():
                age_flag = " UNDERAGE" if result.get("is_underage") else ""
                face_info = f"{result['gender']}, ~{result['age']}y ({result['gender_confidence']:.2f}){age_flag}"
        ...
        instructions = [
            ...
            "G - Toggle Age/Gender Checks" if self.enable_gender_detection else "",
            "Q - Quit"
        ]
    ...
    def run(self):
        ...
        print("Controls: SPACE=Record, B=Toggle Boxes, P=Privacy Mode, G=Age/Gender Checks, Q=Quit")
        ...
        try:
            while True:
                ...
                try:
                    processed_frame, detections, face_results, demographics = self.result_queue.get_nowait()
                    self.last_detections = detections
                    self.face_analysis_results = face_results
                ...
                try:
                    ...
                    if self.latest_protection_policy.get("blocked"):
                        display_frame = np.zeros_like(frame)
                        cv2.putText(
                            display_frame,
                            "SAFEVISION CHILD-PROTECTION BLOCK",
                            (max(20, frame.shape[1] // 2 - 310), frame.shape[0] // 2),
                    ...
                    if (LIVE_CONFIG['AUTO_RECORD_ON_DETECTION'] and 
                        (len(detections) > 0 or self.latest_protection_policy.get("blocked")) and not self.recording):
                ...
                if key == ord('q'):
                    ...
                    print(f"Age/gender checks: {'ON' if self.enable_gender_detection else 'OFF'}")
...
def parse_args():
    ...
    parser.add_argument("-g", "--gender-detection", "--demographics", dest="gender_detection", action="store_true", default=True,
                       help="Enable age and gender checks (enabled by default)")
    parser.add_argument("--no-demographics", dest="gender_detection", action="store_false",
                       help="Disable both age and gender checks")
    ...
    parser.add_argument("--underage-age", type=float, default=None,
                       help="Estimated underage threshold; defaults to the rule file (18)")
    parser.add_argument("--age-review-margin", type=float, default=None,
                       help="Review band above the threshold; defaults to the rule file (3)")
