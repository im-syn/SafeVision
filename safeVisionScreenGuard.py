...
class ScreenGuard:
    ...
    def draw_overlay(self, detections, frame=None):
        ...
        self._tk_images = []
        if not detections and not self.args.show_status:
            self.hide_overlay()
            return

        self.show_overlay()

        if detections and self.args.privacy_on_detection:
            ...
            self.canvas.create_text(
                ...
                text="SafeVision blocked unsafe screen content",
                fill="white",
        ...
        if self.args.show_status:
            ...
            self.canvas.create_rectangle(8, 8, 700, 34, fill=self.label_bg, outline="")
...
def parse_args():
    ...
    parser.add_argument("--rules", type=str, default="BlurException.rule", help="BlurException.rule file path.")
